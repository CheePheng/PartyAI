...
export const GameMenu: React.FC<GameMenuProps> = ({ onSelectGame, onBack, lang }) => {
  ...
  return (
    <div className="flex flex-col h-full animate-fade-in relative">
       ...
       <div className="absolute bottom-10 left-10 w-64 h-64 bg-purple-500/10 rounded-full blur-3xl pointer-events-none -z-10" />
         ...
         <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 px-2">
           {games.map((game, index) => (
             <button
               ...
               className="group relative flex flex-col text-left w-full h-auto min-h-[160px] rounded-[2rem] overflow-hidden transition-all duration-300 hover:scale-[1.02] active:scale-[0.98] ring-1 ring-white/5 hover:ring-white/20 shadow-xl hover:shadow-2xl animate-slide-in"
               style={{ animationDelay: `${index * 50}ms`, animationFillMode: 'both' }}
             >
               {/* Animated Gradient Background */}
               <div className={`absolute inset-0 bg-gradient-to-br ${game.color} opacity-20 group-hover:opacity-30 transition-opacity duration-500`} />
               ...
               <div className="relative z-10 p-5 flex flex-col h-full">
                 <div className="flex justify-between items-start mb-3">
                    <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-md flex items-center justify-center text-3xl shadow-inner border border-white/10 group-hover:scale-110 group-hover:rotate-3 transition-transform duration-300">
                        {game.icon}
                    </div>
                 ...
                 <h3 className="text-xl font-black text-white mb-1 tracking-tight group-hover:translate-x-1 transition-transform duration-300">
                    {game.title[lang]}
                 </h3>
                 ...
                 <div className="mt-auto flex items-center justify-between border-t border-white/5 pt-3">
                    ...
                    <div className="w-8 h-8 rounded-full bg-white/5 flex items-center justify-center text-gray-500 group-hover:bg-white group-hover:text-indigo-600 transition-all duration-300 shadow-sm group-hover:shadow-lg group-hover:shadow-white/20">
